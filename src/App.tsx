...
const playTypingSound = (switchType: string) => {
  ...
  if (switchType === 'linear') {
    ...
    // Topre (Electro-capacitive) - Soft, deep thwomp, rubber dome collapse
    const osc = audioCtx.createOscillator();
    const gain = audioCtx.createGain();
    osc.type = 'sine';
    ...
    // Silent Linear (Silent Alpaca) - Very muted mushy bottom out
    const osc = audioCtx.createOscillator();
    const gain = audioCtx.createGain();
    osc.type = 'sine';
    ...
    // Heavy Tactile (Boba U4T) - Sharp, massive bump, no pretravel thock
    const osc = audioCtx.createOscillator();
    const gain = audioCtx.createGain();
    osc.type = 'triangle';
